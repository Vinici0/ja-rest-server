const calculateAndUpdateMedidas = async (
  ...
  try {
    await calculateAndUpdateAlcantarilladoMeidaIgualACero(
      medidas[0],
      dbConnection,
      sequelize,
      ja_tabla
    ...
    for (let i = 1; i < medidas.length; i++) {
      ...
      if (monthsDifference > 0) {
        ...
        if (medidas[i].Mes >= 8 && medidas[i].Anio >= 2023) {
        ...
        const totalSaldo =
          Number(medidas[i].Saldo) + Number(totalAlcantarillado);
...
const calculateAndUpdateAlcantarilladoMeidaIgualACero = async (
  ...
  const Total = medida.Basico + ExcedenteV;
  const Pago = 0;

  //TODO: Corregir la parte del mes

  let Alcantarillado = medida.Basico === 2.75 ? 1.5 : 3.0;
  const EditarMedida = await dbConnection.query(
    ...
    {
      replacements: {
        LecturaActual: medida.LecturaActual,
        Excedente: Excedente,
